...
public class IssueBook extends JFrame implements ActionListener{
	...
	panel.setBorder(new TitledBorder(new LineBorder(new Color(47, 79, 79), 2, true), "Chi tiết sách",
	...
	t8.setFont(new Font("Time new roman", Font.BOLD, 14));
	t8.setColumns(10);
	...
	t9.setFont(new Font("Time new roman", Font.BOLD, 14));
	...
	t10.setFont(new Font("Time new roman", Font.BOLD, 14));
	...
	t11.setFont(new Font("Time new roman", Font.BOLD, 14));
	...
	t12.setFont(new Font("Time new roman", Font.BOLD, 14));
	...
	t13.setFont(new Font("Time new roman", Font.BOLD, 14));
	...
	t14.setFont(new Font("Time new roman", Font.BOLD, 14));
	...
	panel_1.setBorder(new TitledBorder(new LineBorder(new Color(70, 130, 180), 2, true), "Chi tiết sinh viên",
			TitledBorder.LEADING, TitledBorder.TOP, null, new Color(100, 149, 237)));
	panel_1.setForeground(new Color(0, 100, 0));
	panel_1.setBounds(366, 38, 378, 372);
        panel_1.setBackground(Color.WHITE);
	contentPane.add(panel_1);
        
	JLabel l15 = new JLabel(" Ngày mượn :");
	l15.setForeground(new Color(105, 105, 105));
	l15.setFont(new Font("Time new roman", Font.BOLD, 15));
	...
	b3.setFont(new Font("Time new roman", Font.BOLD, 14));
	...
	b4.setFont(new Font("Time new roman", Font.BOLD, 14));
...
//            }
		...
		ResultSet rs = st.executeQuery();
                ...
                while (rs.next()) {
                    ...
                    t5.setText(rs.getString("number"));
		...
		ResultSet rs = st.executeQuery();
                ...
                while (rs.next()) {
                    ...
                    t12.setText(rs.getString("Nganhhoc"));
		...
		rs.close();
            ...
            if(ae.getSource() == b3){
                ...
                else if(t1.getText().contains("N") && t11.getText().contains("HDS")){
                       JOptionPane.showMessageDialog(this, "Bạn không thể mượn sách hệ quân sự");
                   }else{
                        try{
                String sql = "insert into issueBook(book_id, student_id, bname, sname, course, Nganhhoc, dateOfIssue) values(?, ?, ?, ?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
		st.setString(1, t1.getText());
		st.setString(2, t8.getText());
		st.setString(3, t2.getText());
		...
		if (i > 0){
                        ...
                        new BookDetails().setVisible(false);
    ...
    public int updateBook(){
        ...
        try {
            conn con = new conn();
            String sql = "update book set number= ? " + " where book_id = '"+t1.getText()+"'";
            PreparedStatement st = con.c.prepareStatement(sql);
            int giam = Integer.parseInt(t5.getText()) - 1;
            String text = String.valueOf(giam);
            st.setString(1,text);
        i = st.executeUpdate();
